...
function expandRequest(event, remove = false) {
	...
	if (searchTypeParsed === 'ov') {
		...
		$.ajax({
			type: 'GET',
			url: '.',
			data: 's=' + searchTerm + '&t=' + searchTypeParsed + '&st=norm2'
		}).done(function (data) {
		...
		$.ajax({
			type: 'GET',
			url: '.',
			data: 's=' + currentSearchTerm + '&t=' + searchTypeParsed + '&o=' + offset + '&st=stframe'
		}).done(function (data) {
		...
		$.ajax({
			type: 'GET',
			url: '.',
			data: 's=' + currentSearchTerm + '&o=' + offset + '&t=' + searchTypeParsed + '&c=' + pos + '&st=exp'
		}).done(function (data) {
			expandedSearchFormatter(data, event, remove)
		});
	}
...
function search(searchTerm) {
	...
	var result = $.ajax({
		type: 'GET',
		url: '.',
		data: 's=' + String(searchTerm) + '&st=norm1'
	}).done(function (data) {
		if (data.found === 1) {
...
function advancedSearchLangs(event, classKnown = false) {
	...
	if ($(".langmenu").select2('data').length !== 0) {
		$.ajax({
			type: 'GET',
			url: '.',
			data: 'o=' + String(offset) + '&langs=' + JSON.stringify(otherLangs) + '&c=' + pos + '&st=advsearch'
		}).done(function (data) {
	...
	$(".langmenu").on('select2:select', function (e) {
		...
		if (currentTranslatedOffset !== undefined) {
			...
			$.ajax({
				type: 'GET',
				url: '.',
				data: 'o=' + String(offset) + '&langs=' + JSON.stringify([e.params.data.id]) + '&c=' + pos + '&st=advsearch'
			}).done(function (data) {
