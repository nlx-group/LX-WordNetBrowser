...
const relationsShort = {
	'DirectHypernym': 'dhype',
	'DirectHyponym': 'dhypo',
	'DirectTroponym': 'dhypo',
	'InheritedHypernym': 'ihype',
	'FullHyponym': 'fhypo',
	'FullTroponym': 'fhypo',
	'MemberHolonym': 'mh',
	'SubstanceHolonym': 'sh',
	...
	'PartMeronym': 'pm',
	'Antonym': 'ant',
	'InstanceHypernym': 'inhype',
	'InstanceHyponym': 'inhypo',
	'Attribute': 'attr',
	'DerivationallyRelatedForm': 'derform',
	'Entailment': 'ent',
	'Cause': 'ca',
	'AlsoSee': 'asee',
	'VerbGroup': 'vgr',
	'SimilarTo': 'sto',
	'ParticipleOfVerb': 'pverb',
	'Pertainym': 'per',
	'DomainCategory': 'domcat',
	'DomainTermCategory': 'domtermcat',
	'DomainRegion': 'domreg',
	'DomainTermRegion': 'domtermreg',
	'DomainUsage': 'domusage',
	'DomainTermUsage': 'domtermusage',
	'Overview': 'ov',
	'Concept': 'con',
	'SentenceFrame': 'sframe'
};
const partOfSpeech = {
	'n': 'noun',
	'v': 'verb',
...
function expandedSearchFormatter(data, event, remove) {
	...
	if (event.target.className === 'DirectHypernym' || event.target.className === 'DirectHyponym' || event.target.className === 'InheritedHypernym' || event.target.className === 'FullHyponym' || event.target.className === 'DirectTroponym' || event.target.className == 'FullTroponym') {
		$(event.target.parentNode.children[2]).find('a').click(function (event) {
			expand(event)
		});
		...
		$(event.target.parentNode.children[1]).find('a').click(function (event) {
			expand(event)
		});
		$("span.mark").each(function () {
			var lemma = $(this).text();
...
function expandRequest(event, remove = false) {
	...
	if (searchTypeParsed === 'ov') {
		...
		$.ajax({
			type: 'GET',
			url: '.',
			data: 's=' + searchTerm + '&t=' + searchTypeParsed + '&st=norm2' + '&language=' + currentSearchLanguage
		}).done(function (data) {
			expandedSearchFormatter(data, event, remove)
		});
		...
		$.ajax({
			type: 'GET',
			url: '.',
			data: 's=' + currentSearchTerm + '&t=' + searchTypeParsed + '&o=' + offset + '&st=stframe' + '&language=' + currentSearchLanguage
		}).done(function (data) {
			sentenceFrameFormatter(data, event)
		});
	} else {
		...
		$.ajax({
			type: 'GET',
			url: '.',
			data: 's=' + currentSearchTerm + '&o=' + offset + '&t=' + searchTypeParsed + '&c=' + pos + '&st=exp' + '&language=' + currentSearchLanguage
		}).done(function (data) {
			expandedSearchFormatter(data, event, remove)
		});
	}
...
function expand(event) {
	if ($(event.target.parentNode).find("ul.expand").length === 0) {
		...
		var relationsMenuLines = {
			'@': '<li><a class="DirectHypernym" style="cursor:pointer">' + relationsMenu['directHypernym'] + '</a> | <a class="InheritedHypernym" style="cursor:pointer">' + relationsMenu['inheritedHypernym'] + '</a></li>',
			...
			'#m': '<li><a class="MemberHolonym" style="cursor:pointer">' + relationsMenu['memberHolonym'] + '</a></li>',
			'#s': '<li><a class="SubstanceHolonym" style="cursor:pointer">' + relationsMenu['substanceHolonym'] + '</a></li>',
			'#p': '<li><a class="PartHolonym" style="cursor:pointer">' + relationsMenu['partHolonym'] + '</a></li>',
			'%m': '<li><a class="MemberMeronym" style="cursor:pointer">' + relationsMenu['memberMeronym'] + '</a></li>',
			'%s': '<li><a class="SubstanceMeronym" style="cursor:pointer">' + relationsMenu['substanceMeronym'] + '</a></li>',
			'%p': '<li><a class="PartMeronym" style="cursor:pointer">' + relationsMenu['partMeronym'] + '</a></li>',
			'!': '<li><a class="Antonym" style="cursor:pointer">' + relationsMenu['antonym'] + '</a></li>',
			'@i': '<li><a class="InstanceHypernym" style="cursor:pointer">' + relationsMenu['instanceHypernym'] + '</a></li>',
			'~i': '<li><a class="InstanceHyponym" style="cursor:pointer">' + relationsMenu['instanceHyponym'] + '</a></li>',
			'=': '<li><a class="Attribute" style="cursor:pointer">' + relationsMenu['attribute'] + '</a></li>',
			'+': '<li><a class="DerivationallyRelatedForm" style="cursor:pointer">' + relationsMenu['derivationallyRelatedForm'] + '</a></li>',
			'*': '<li><a class="Entailment" style="cursor:pointer">' + relationsMenu['entailment'] + '</a></li>',
			'>': '<li><a class="Cause" style="cursor:pointer">' + relationsMenu['cause'] + '</a></li>',
			'^': '<li><a class="AlsoSee" style="cursor:pointer">' + relationsMenu['alsoSee'] + '</a></li>',
			'$': '<li><a class="VerbGroup" style="cursor:pointer">' + relationsMenu['verbGroup'] + '</a></li>',
			'&': '<li><a class="SimilarTo" style="cursor:pointer">' + relationsMenu['similarTo'] + '</a></li>',
			'<': '<li><a class="ParticipleOfVerb" style="cursor:pointer">' + relationsMenu['participleOfVerb'] + '</a></li>',
			'\\': ['<li><a class="Pertainym" style="cursor:pointer">' + relationsMenu['pertainym'] + '</a></li>',
				'<li><a class="Pertainym" style="cursor:pointer">' + relationsMenu['derivedFromAdjective'] + '</a></li>'
			],
			';c': '<li><a class="DomainCategory" style="cursor:pointer">' + relationsMenu['domainCategory'] + '</a></li>',
			'-c': '<li><a class="DomainTermCategory" style="cursor:pointer">' + relationsMenu['domainTermCategory'] + '</a></li>',
			';r': '<li><a class="DomainRegion" style="cursor:pointer">' + relationsMenu['domainRegion'] + '</a></li>',
			'-r': '<li><a class="DomainTermRegion" style="cursor:pointer">' + relationsMenu['domainTermRegion'] + '</a></li>',
			';u': '<li><a class="DomainUsage" style="cursor:pointer">' + relationsMenu['domainUsage'] + '</a></li>',
			'-u': '<li><a class="DomainTermUsage" style="cursor:pointer">' + relationsMenu['domainTermUsage'] + '</a></li>',
			'overview': '<li><a class="Overview" style="cursor:pointer">' + relationsMenu['overview'] + '</a></li>',
			'concept': '<li><a class="Concept" style="cursor:pointer">' + relationsMenu['concept'] + '</a></li>',
...
function formattedResults(result) {
	...
	$("#resultList a").click(function (event) {
		expand(event)
	});
	$("span.mark").each(function () {
		var lemma = $(this).text();
...
function languageCollisionMenu(languages) {
	...
	$("#collisionSelect").on('select2:select', function (e) {
		$.ajax({
			type: 'GET',
			url: '.',
			data: 's=' + String(currentSearchTerm) + '&st=norm1' + '&language=' + e.params.data.id
		}).done(function (data) {
			if (data.found === 1) {
...
function search(searchTerm) {
	...
	var result = $.ajax({
		type: 'GET',
		url: '.',
		data: 's=' + String(searchTerm) + '&st=norm1' + '&language=UNK'
	}).done(function (data) {
		if (data.collision === 1) {
			languageCollisionMenu(data.languages)
...
function advancedSearchLangs(event, classKnown = false) {
	...
	if ($(".langmenu").select2('data').length !== 0) {
		$.ajax({
			type: 'GET',
			url: '.',
			data: 'o=' + String(offset) + '&langs=' + JSON.stringify(otherLangs) + '&c=' + pos + '&st=advsearch' + '&language=' + currentSearchLanguage
		}).done(function (data) {
			advancedSearchLangsAppender(data.result)
		});
	}
}
...
function main() {
	...
	$("#normal-search").click(function () {
		...
		$("#pLangList").offset({
			left: String($("#searchInput").offset().left)
		});
	...
	$(".langmenu").on('select2:select', function (e) {
		...
		if (currentTranslatedOffset !== undefined) {
			...
			$.ajax({
				type: 'GET',
				url: '.',
				data: 'o=' + String(offset) + '&langs=' + JSON.stringify([e.params.data.id]) + '&c=' + pos + '&st=advsearch' + '&language=' + currentSearchLanguage
			}).done(function (data) {
				advancedSearchSingleLang(data.result)
			});
		}
	});
	...
	$(".invis").width($("#langSelectList").width()).height($("#langSelectList").height()).css({
		'border-radius': $("#langSelectList").css('border-radius'),
		'-webkit-border-radius': $("#langSelectList").css('-webkit-border-radius'),
		'-moz-border-radius': $("#langSelectList").css('-moz-border-radius')
	});
	...
	$("#Node").offset({
		...
		top: $("img.headerImage").offset().top + 125
	});
